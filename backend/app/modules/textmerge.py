...
def _find_overlap(prev: list[str], nxt: list[str]) -> int:
    limit = min(_CHECK, len(prev), len(nxt))
    for k in range(limit, 0, -1):
        tail, head = prev[-k:], nxt[:k]
        if all(fuzz.ratio(norm(a), norm(b)) >= 80 for a, b in zip(tail, head)):
            return k
    return 0
...
def merge_lists(segments: list[list[str]]) -> list[str]:
    ...
    merged: list[str] = []
    for seg in segments:
        seg = [s for s in seg if s and s.strip()]
        if not merged:
            merged.extend(seg)
            continue
        merged.extend(seg[_find_overlap(merged, seg) :])
    return merged
